...
class Event implements Serializable {
	...
	public String toString() {
		return "[title=" + title + "" + a2.format(startTime) + "~" + a2.format(lastTime) + ", details=" + details
				+ "]";
	}
...
public class Project_date {
	...
	private static void saveEvents(Map<String, List<Event>> events) throws IOException {
		ObjectOutputStream writer = new ObjectOutputStream(new FileOutputStream(name+"2.ser"));
	...
	private static void deleteEvent(Map<String, List<Event>> events) throws ParseException {
		...
		try {
			...
			if (input.equalsIgnoreCase("y")) {
				System.out.println(sameDateEvents.get(choice - 1)+" 삭제");
